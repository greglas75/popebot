...
class TelegramAdapter extends ChannelAdapter {
  ...
  async receive(request) {
    ...
    if (!headerSecret ||
        Buffer.byteLength(headerSecret) !== Buffer.byteLength(TELEGRAM_WEBHOOK_SECRET) ||
        !timingSafeEqual(Buffer.from(headerSecret), Buffer.from(TELEGRAM_WEBHOOK_SECRET))) {
      return null;
    }
    ...
    if (message.document) {
      ...
      if (message.document.file_size && message.document.file_size > MAX_DOC_SIZE) {
        try { await sendMessage(this.botToken, chatId, 'Document too large (max 5 MB).'); } catch {}
      } else {
        try {

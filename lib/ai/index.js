...
async function* persistentContainerStream(threadId, message, workspaceId, options) {
  ...
  const { status: containerStatus, containerName } = await ensureContainer(workspaceId, { codingAgent });
  ...
  if (containerStatus !== 'ready') {
    ...
    if (hasLLM) {
      try {
        ...
        if (text) {
          yield { type: 'text', text };
          persistMessage(threadId, 'assistant', text, options);
        }
      } catch (err) {
    ...
    try {
      ...
      yield { type: 'text', text: `Failed to start agent container: ${err.message}` };
      return;
    }
  }

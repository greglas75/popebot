...
async function* persistentContainerStream(threadId, message, workspaceId, options) {
  ...
  try {
    ...
    console.error('[persistentContainerStream] ensureContainer failed:', err);
  ...
  if (containerStatus === 'error' || !containerName) {
    yield { type: 'text', text: 'Failed to start coding agent container. Please try again.' };
    return;
  }
  ...
  try {
    ...
    console.error('[persistentContainerStream] Failed:', err);

...
describe('mapLine edge cases', () => {
  ...
  it('handles JSON null as unknown without calling mapper', () => {
    expect(mapLine('null')).toEqual([{ type: 'unknown', raw: null }]);
  });
});
...
describe('mapClaudeCodeLine null content edge cases', () => {
  ...
  it('handles rate_limit_event with no rate_limit_info at all', () => {
    ...
    expect(events[0].type).toBe('text');
    expect(events[0].text).toContain('Rate limited (request)');
    expect(events[0].text).toContain(new Date(0).toISOString());
  });

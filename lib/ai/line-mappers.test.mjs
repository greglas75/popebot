...
describe('mapLine', () => {
  ...
  it('filters out skip events but keeps other events on the same line', () => {
...
describe('mapClaudeCodeLine', () => {
  ...
  it('surfaces rate_limit_event with status blocked as text with timestamp', () => {
    ...
    expect(events[0].text).toContain(new Date(resetsAt * 1000).toISOString());
  });
  ...
  it('does not apply root tool_use_result.stdout when multiple tool_results are present', () => {
    const events = mapClaudeCodeLine({
      type: 'user',
      message: {
        content: [
      ...
      tool_use_result: { stdout: 'shared stdout should not overwrite both' },
    });
    expect(events).toEqual([
      { type: 'tool-result', toolCallId: 'a', result: 'result A' },
      { type: 'tool-result', toolCallId: 'b', result: 'result B' },
    ]);
  });
...
describe('mapPiLine', () => {
  ...
  it('returns skip for session events', () => {
    const events = mapPiLine({ type: 'session' });
    expect(events).toEqual([{ type: 'skip' }]);
  });

  it('returns skip for turn_start events', () => {
    const events = mapPiLine({ type: 'turn_start' });
    expect(events).toEqual([{ type: 'skip' }]);
  });

  it('returns skip for message_update without assistantMessageEvent', () => {
...
describe('mapLine integration', () => {
  ...
  it('suppresses Pi unhandled types through mapLine (skip), not unknown', () => {
    const line = JSON.stringify({ type: 'totally_alien' });
    const events = mapLine(line, mapPiLine);
    expect(events).toHaveLength(0);
  });

  it('emits unknown for unhandled Claude stream line shapes', () => {
    const line = JSON.stringify({ type: 'totally_unknown_claude_shape' });
    const events = mapLine(line);
    expect(events).toEqual([{ type: 'unknown', raw: { type: 'totally_unknown_claude_shape' } }]);
  });
});

// ─────────────────────────────────────────────────────────────────────────────
// Edge cases from review (null/undefined, JSON primitives, content:null)
// ─────────────────────────────────────────────────────────────────────────────

describe('mapLine edge cases', () => {
  it('handles empty string input as non-JSON', () => {
    const events = mapLine('');
    expect(events[0].type).toBe('text');
    expect(events[0].text).toBe('\n\n');
  });

  it('handles JSON primitive (number) — falls through to unknown', () => {
    expect(mapLine('123')).toEqual([{ type: 'unknown', raw: 123 }]);
  });

  it('handles JSON array — falls through to unknown', () => {
    expect(mapLine('[1,2,3]')).toEqual([{ type: 'unknown', raw: [1, 2, 3] }]);
  });

  it('handles JSON null — mapper crashes, wraps as non-JSON text', () => {
    // JSON.parse('null') = null → mapClaudeCodeLine destructures null → throws
    // mapLine does NOT catch mapper errors — this is a production edge case
    // Documenting actual behavior: TypeError thrown
    expect(() => mapLine('null')).toThrow(TypeError);
  });
});

describe('isEmptyEvent null/undefined variants', () => {
  it('catches text event with null text', () => {
    expect(mapLine('{}', () => [{ type: 'text', text: null }])).toEqual([{ type: 'unknown', raw: {} }]);
  });

  it('catches text event with undefined text', () => {
    expect(mapLine('{}', () => [{ type: 'text' }])).toEqual([{ type: 'unknown', raw: {} }]);
  });

  it('catches tool-call with null toolName', () => {
    expect(mapLine('{}', () => [{ type: 'tool-call', toolCallId: 'x', toolName: null }])).toEqual([{ type: 'unknown', raw: {} }]);
  });

  it('catches tool-result with null toolCallId', () => {
    expect(mapLine('{}', () => [{ type: 'tool-result', toolCallId: null, result: 'data' }])).toEqual([{ type: 'unknown', raw: {} }]);
  });
});

describe('mapClaudeCodeLine null content edge cases', () => {
  it('returns empty for assistant with content: null', () => {
    expect(mapClaudeCodeLine({ type: 'assistant', message: { content: null } })).toHaveLength(0);
  });

  it('returns empty for user with content: null', () => {
    expect(mapClaudeCodeLine({ type: 'user', message: { content: null } })).toHaveLength(0);
  });

  it('handles rate_limit_event with no rate_limit_info at all', () => {
    const events = mapClaudeCodeLine({ type: 'rate_limit_event' });
    expect(events[0].type).toBe('text');
    expect(events[0].text).toContain('Rate limited (undefined)');
  });

  it('skips text blocks with null text in assistant message', () => {
    expect(mapClaudeCodeLine({
      type: 'assistant',

...
/** Deterministic reset time for rate-limit UI (avoids locale-dependent toLocaleString). */
function formatRateLimitReset(resetsAtUnixSec) {
  const ms = (resetsAtUnixSec || 0) * 1000;
...
export function mapLine(line, mapper = mapClaudeCodeLine) {
  ...
  // JSON null / primitives / arrays — do not call mapper (avoids TypeError on destructuring null)
  if (parsed === null || typeof parsed !== 'object' || Array.isArray(parsed)) {
    return [{ type: 'unknown', raw: parsed }];
  }

  const events = mapper(parsed);
  const filtered = events.filter(e => e.type !== 'skip');
  if (filtered.length === 0) {
    // Mapper returned only skips, or nothing — distinguish unknown (unhandled shape) vs suppress
    if (events.length === 0) {
      return [{ type: 'unknown', raw: parsed }];
    }
    return [];
  }
  if (filtered.every(isEmptyEvent)) {
    return [{ type: 'unknown', raw: parsed }];
  }
  return filtered;
}
...
export function mapClaudeCodeLine(parsed) {
  ...
  // Rate limit events — always skip (internal info, not user-facing)
  if (type === 'rate_limit_event') {
    const info = parsed.rate_limit_info;
    if (info?.status !== 'allowed') {
      console.warn(`[rate-limit] ${info?.rateLimitType ?? 'request'} — resets at ${formatRateLimitReset(info?.resetsAt)}`);
    }
    return [{ type: 'skip' }];
  }
  ...
  if (type === 'assistant' && message?.content) {
    ...
    const toolBlocks = message.content.filter(b => b.type === 'tool_result');
    const useRootStdout = tool_use_result?.stdout != null && toolBlocks.length === 1;
    for (const block of toolBlocks) {
      const resultText = useRootStdout
    ...
    // Result is the final summary — skip to avoid duplicating assistant text
    // (assistant messages already emitted the content as it streamed)
    return [{ type: 'skip' }];
  }

  ...
  try {
    ...
    if (isPrerelease(installed)) {
      ...
      const results = await Promise.allSettled([
        fetch('https://registry.npmjs.org/thepopebot/latest', { signal: AbortSignal.timeout(10_000) }),
        fetch('https://registry.npmjs.org/thepopebot/beta', { signal: AbortSignal.timeout(10_000) }),
      ]);
      ...
      if (checkBeta) {
        ...
        const res = await fetch('https://registry.npmjs.org/thepopebot/latest', { signal: AbortSignal.timeout(10_000) });
        if (!res.ok) {
...
/**
 * Start built-in crons (version check, idle container management). Called from instrumentation.
 */
function startBuiltinCrons() {
  ...
  // Manage idle containers every 5 minutes
  cron.schedule('*/5 * * * *', () => {
    console.log('[cron] Managing idle containers...');
    import('./containers/lifecycle.js')
      .then(({ manageIdleContainers }) => manageIdleContainers())
      .then(() => console.log('[cron] Idle container management complete'))
      .catch((err) => console.error(`[cron] Idle container management failed: ${err.message}`));
  });
}

 ...
 *
 * For claude-code: tries tmux session approach first (instant, no process restart).
 * Falls back to docker exec claude -p for other agents or if tmux is unavailable.
 *
...
export async function* execPrompt(containerName, codingAgent, prompt, options = {}) {
  // Try tmux session approach for claude-code (instant, plugins loaded once)
  if (codingAgent === 'claude-code') {
    try {
      yield* execPromptViaTmux(containerName, prompt, options);
      return;
    } catch (err) {
      console.warn(`[execPrompt] tmux approach failed (${err.message}), falling back to exec`);
    }
  }
...
/**
 * Send a prompt to the running Claude Code tmux session and stream back
 * parsed events from the session JSONL file.
 *
 * How it works:
 *   1. Verify tmux session is alive and Claude Code is the active process
 *   2. Find the session JSONL file and record current line count
 *   3. Send prompt via tmux load-buffer + paste-buffer (safe for special chars)
 *   4. Tail the session JSONL for new entries
 *   5. Map JSONL entries to chat events (text, tool-call, tool-result)
 *   6. Stop when assistant responds with stop_reason !== 'tool_use'
 *
 * @throws {Error} TMUX_UNAVAILABLE if tmux session is dead or Claude Code is not running
 */
// Per-workspace mutex for tmux sends
const tmuxLocks = new Map();

async function* execPromptViaTmux(containerName, prompt, options = {}) {
  const { execInContainer, execStreamInContainer } = await import('../tools/docker.js');

  // Serialize concurrent sends to the same container
  const prev = tmuxLocks.get(containerName) || Promise.resolve();
  let releaseLock;
  const lock = new Promise((r) => { releaseLock = r; });
  tmuxLocks.set(containerName, lock);
  await prev;

  let stream;
  try {
    // Helper: exec as coding-agent user (tmux server is per-user)
    const asUser = (cmd, timeout = 5000) => execInContainer(containerName, cmd, timeout, 'coding-agent');

    // 1. Check tmux session + Claude Code is alive
    const paneCmd = await asUser("tmux list-panes -t claude -F '#{pane_current_command}' 2>/dev/null");
    if (!paneCmd || !paneCmd.trim().includes('claude')) {
      throw new Error(`TMUX_UNAVAILABLE: pane=${JSON.stringify(paneCmd)}`);
    }

    // 2. Find session JSONL (most recently modified)
    const sessionFile = await asUser("ls -t /home/coding-agent/.claude/projects/-home-coding-agent-workspace/*.jsonl 2>/dev/null | head -1");
    const jsonlPath = sessionFile?.trim();
    if (!jsonlPath) {
      throw new Error('TMUX_UNAVAILABLE: no session file');
    }

    // Record line count BEFORE sending prompt
    const lcStr = await asUser(`wc -l < '${jsonlPath}'`);
    const startLine = parseInt(lcStr?.trim() || '0') + 1;

    // 3. Handle model switching via /model command
    if (options.model) {
      await asUser(`tmux send-keys -t claude '/model ${options.model}' Enter`);
      await new Promise(r => setTimeout(r, 500));
    }

    // 4. Send prompt safely via file buffer (handles special chars, newlines)
    const b64 = Buffer.from(prompt, 'utf-8').toString('base64');
    await asUser(
      `echo '${b64}' | base64 -d > /tmp/claude-prompt.txt && tmux load-buffer /tmp/claude-prompt.txt && tmux paste-buffer -t claude && tmux send-keys -t claude Enter`, 10000);

    console.log(`[tmux] sent prompt to ${containerName}, tailing JSONL from line ${startLine}`);

    // 5. Tail session JSONL for new entries (Docker exec stream)
    stream = await execStreamInContainer(containerName,
      ['bash', '-c', `tail -f -n +${startLine} '${jsonlPath}'`]);

    // 6. Parse Docker multiplexed stream → lines → events
    const { DockerFrameParser } = await import('../tools/docker.js');
    ...
    for await (const chunk of stream) {
      ...
      for (const frame of frames) {
        ...
        // Split on newlines — each complete line is a JSON entry
        let nlIdx;
        while ((nlIdx = lineBuf.indexOf('\n')) !== -1) {
          ...
          const events = mapSessionEntry(entry);
          for (const event of events) {
            gotResponse = true;
            yield event;
          }

          // Response complete when assistant finishes (not mid-tool-use)
    ...
    if (!gotResponse) {
      throw new Error('TMUX_NO_RESPONSE');
    }
  } finally {
    if (stream?.destroy) stream.destroy();
    releaseLock();
    if (tmuxLocks.get(containerName) === lock) tmuxLocks.delete(containerName);
  }
}

/**
 * Map a session JSONL entry to chat events.
 * Session JSONL format: { type: 'assistant'|'user', message: { role, content: [...] } }
 */
function mapSessionEntry(entry) {
  ...
  // User messages with tool results (Claude Code ran a tool internally)
  ...
  // Skip: queue-operation, attachment, last-prompt, summary, etc.
  return [];
}

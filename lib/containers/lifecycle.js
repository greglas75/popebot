...
async function* execPromptViaTmux(containerName, prompt, options = {}) {
  ...
  try {
    ...
    const asUser = (cmd) => execInContainer(containerName, `su - coding-agent -c '${cmd.replace(/'/g, "'\\''")}'`, 5000);
    ...
    if (!paneCmd || !paneCmd.trim().includes('claude')) {
      throw new Error('TMUX_UNAVAILABLE');
    }
    ...
    if (!jsonlPath) {
      throw new Error('TMUX_UNAVAILABLE'); // no session file yet
    }
    ...
    await execInContainer(containerName,
      `echo '${b64}' | base64 -d > /tmp/claude-prompt.txt && su - coding-agent -c "tmux load-buffer /tmp/claude-prompt.txt && tmux paste-buffer -t claude && tmux send-keys -t claude Enter"`, 10000);

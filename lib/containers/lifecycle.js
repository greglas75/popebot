...
async function* execPromptViaTmux(containerName, prompt, options = {}) {
  ...
  try {
    ...
    const paneCmd = await execInContainer(containerName,
      "tmux list-panes -t claude -F '#{pane_current_command}' 2>/dev/null", 5000);
    if (!paneCmd || !paneCmd.trim().includes('claude')) {
      throw new Error('TMUX_UNAVAILABLE');
    }

    // 2. Find session JSONL (most recently modified)
    const sessionFile = await execInContainer(containerName,
      "ls -t /home/coding-agent/.claude/projects/-home-coding-agent-workspace/*.jsonl 2>/dev/null | head -1", 3000);
    ...
    const lcStr = await execInContainer(containerName, `wc -l < '${jsonlPath}'`, 3000);
    const startLine = parseInt(lcStr?.trim() || '0') + 1;

    // 3. Handle model switching via /model command
    if (options.model) {
      await execInContainer(containerName,
        `tmux send-keys -t claude '/model ${options.model}' Enter`, 5000);

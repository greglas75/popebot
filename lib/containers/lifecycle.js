...
export async function auditContainersOnStartup() {
  ...
  for (const ws of workspaces) {
    if (!ws.containerName) continue;
    audited++;
    try {
      const info = await inspectContainer(ws.containerName);
      if (!info) {
        // Container missing from Docker — reset to none
        updateContainerStatus(ws.id, 'none');
        fixed++;
      ...
      console.error(`[startup-audit] Error auditing ${ws.containerName}: ${err.message}`);
    }
  }

  console.log(`[startup-audit] Audited ${audited} containers, fixed ${fixed} status mismatches`);
}

...
async function* execPromptViaSdk(containerName, prompt, options = {}) {
  ...
  // Make HTTP request to SDK server inside the container (Docker network hostname)
  const response = await new Promise((resolve, reject) => {
    const timer = setTimeout(() => reject(new Error('SDK_TIMEOUT')), 5000);
...
function mapSdkMessage(msg) {
  ...
  if (msg.type === 'result' && msg.result) {
    return [{ type: 'text', text: msg.result }];
  }

  // Skip: system, error, etc.
  return [];
}

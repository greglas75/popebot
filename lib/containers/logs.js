...
export async function GET(request) {
  ...
  // Ownership check — non-admin users can only stream their own workspace containers
  if (session.user.role !== 'admin') {
    try {
      const { getDb } = await import('../db/index.js');
      const { codeWorkspaces } = await import('../db/schema.js');
      const { eq } = await import('drizzle-orm');
      const db = getDb();
      const ws = db.select().from(codeWorkspaces).where(eq(codeWorkspaces.containerName, name)).get();
      if (!ws || ws.userId !== session.user.id) {
        return new Response('Forbidden', { status: 403 });
      }
    } catch (err) {
      console.error('[logs] Ownership check failed:', err.message);

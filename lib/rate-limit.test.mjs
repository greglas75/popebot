...
describe('rateLimit', () => {
  ...
  describe('edge cases', () => {
    ...
    it('treats windowMs <= 0 as 1ms minimum so limiting cannot be accidentally disabled', () => {
      // Same clock tick: first allowed, further calls hit max within 1ms window
      expect(rateLimit('window-zero', 1, 0)).toEqual({ allowed: true });
      expect(rateLimit('window-zero', 1, 0).allowed).toBe(false);
...
describe('rateLimitResponse', () => {
  ...
  it('defaults Retry-After to 1 second when retryAfter is missing or invalid', async () => {

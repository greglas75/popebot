...
async function pauseContainer(containerName) {
  ...
  if (status !== 204) throw new Error(`Failed to pause container ${containerName}: ${status}`);
}
...
async function unpauseContainer(containerName) {
  ...
  if (status !== 204) throw new Error(`Failed to unpause container ${containerName}: ${status}`);
}

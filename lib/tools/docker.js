 ...
 */
function dockerApiStream(method, path, body = null) {
  return new Promise((resolve, reject) => {
    const options = {
      socketPath: '/var/run/docker.sock',
      path,
...
const RESOURCE_DEFAULTS = {
  ...
  MemorySwap: 2 * 1024 * 1024 * 1024,   // No swap beyond RAM
  CpuShares: 1024,
  PidsLimit: 512,
...
async function runContainer({ containerName, image, env = [], workingDir, hostConfig = {} }) {
  // Merge resource-limit defaults (caller overrides take precedence)
  const secOpt = [...(RESOURCE_DEFAULTS.SecurityOpt || []), ...(hostConfig.SecurityOpt || [])];
  hostConfig = { ...RESOURCE_DEFAULTS, ...hostConfig, SecurityOpt: secOpt };
...
async function runInteractiveContainer({ containerName, repo, branch, codingAgent, featureBranch, workspaceId, injectSecrets, continueSession = true }) {
  ...
  // Use fork image (greglas75/popebot) which includes SDK server, fall back to upstream
  const forkImage = `greglas75/popebot:coding-agent-${agent}`;
  const upstreamImage = `stephengpope/thepopebot:coding-agent-${agent}-${version}`;
  const image = agent === 'claude-code' ? forkImage : upstreamImage;
  ...
  if (injectSecrets) {
    ...
    console.warn(`[security] Injecting ${jobSecrets.length} secrets into interactive container ${containerName}`);
  ...
  if (workspaceId) {
    ...
    try { fs.chownSync(dir, CODING_AGENT_UID, CODING_AGENT_UID); } catch {}
    try { fs.chownSync(wsDir, CODING_AGENT_UID, CODING_AGENT_UID); } catch {}
 ...
 * @returns {Promise<{containerId: string, containerName: string}>}
 */
async function runHeadlessContainer({ containerName, repo, branch, featureBranch, workspaceId, taskPrompt, mode = 'plan', codingAgent, modelOverride, systemPrompt, continueSession = true, injectSecrets }) {
  const agent = codingAgent || getConfig('CODING_AGENT') || 'claude-code';
  const version = process.env.THEPOPEBOT_VERSION;
  ...
  // Apply per-chat model override from the UI model selector
  if (modelOverride) {
    let replaced = false;
    for (let i = 0; i < env.length; i++) {
      if (env[i].startsWith('LLM_MODEL=')) {
        env[i] = `LLM_MODEL=${modelOverride}`;
        replaced = true;
        break;
      } else if (env[i].startsWith('ANTHROPIC_MODEL=')) {
        env[i] = `ANTHROPIC_MODEL=${modelOverride}`;
        replaced = true;
        break;
      }
    }
    if (!replaced) env.push(`LLM_MODEL=${modelOverride}`);
  }
  ...
  if (injectSecrets) {
    const { getAllAgentJobSecrets } = await import('../db/config.js');
    const jobSecrets = getAllAgentJobSecrets();
    console.warn(`[security] Injecting ${jobSecrets.length} secrets into headless container ${containerName}`);
    for (const { key, value } of jobSecrets) {
      if (value !== null && !env.some(e => e.startsWith(`${key}=`))) {
        env.push(`${key}=${value}`);
      }
 ...
 */
async function execInContainer(containerName, cmd, timeoutMs = 5000, user = null) {
  try {
    // Create exec instance
    const execConfig = { Cmd: ['sh', '-c', cmd], AttachStdout: true, AttachStderr: false };
    if (user) execConfig.User = user;
...
async function runAgentJobContainer({ agentJobId, repo, branch, title, description, codingAgent, llmModel }) {
  ...
  const { getAllAgentJobSecrets } = await import('../db/config.js');
  const jobSecrets = getAllAgentJobSecrets();
  console.warn(`[security] Injecting ${jobSecrets.length} secrets into agent-job container ${containerName}`);
  for (const { key, value } of jobSecrets) {
    if (value !== null && !env.some(e => e.startsWith(`${key}=`))) {
      env.push(`${key}=${value}`);
    }
...
async function pauseContainer(containerName) {
  const { status } = await dockerApi('POST', `/containers/${encodeURIComponent(containerName)}/pause`);
  if (status !== 204 && status !== 304) throw new Error(`Failed to pause container ${containerName}: ${status}`);
}
...
async function unpauseContainer(containerName) {
  const { status } = await dockerApi('POST', `/containers/${encodeURIComponent(containerName)}/unpause`);
  if (status !== 204 && status !== 304) throw new Error(`Failed to unpause container ${containerName}: ${status}`);
}

/**
 * Create an exec instance in a container and return a streaming response.
 * @param {string} containerName
 * @param {string|string[]} cmd - Command string or array
 * @returns {Promise<http.IncomingMessage>} Raw readable stream of exec output
 */
async function execStreamInContainer(containerName, cmd) {
  const cmdArray = Array.isArray(cmd) ? cmd : ['/bin/bash', '-c', cmd];
  const { status: createStatus, data } = await dockerApi(
    'POST',
    `/containers/${encodeURIComponent(containerName)}/exec`,
    {
      AttachStdout: true,
      ...
      Tty: false,
      User: 'coding-agent',
      WorkingDir: '/home/coding-agent/workspace',
      Env: ['PATH=/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin:/home/coding-agent/.local/bin'],
    }
  );
  if (createStatus !== 201) throw new Error(`Docker exec create failed (${createStatus})`);

  return dockerApiStream('POST', `/exec/${data.Id}/start`, { Detach: false, Tty: false });
}

...
async function runInteractiveContainer({ containerName, repo, branch, codingAgent, featureBranch, workspaceId, injectSecrets, continueSession = true }) {
  ...
  if (workspaceId) {
    ...
    hostConfig.Binds = [`${hostDir}:/home/coding-agent`, ...buildCredentialBinds()];
  }
 ...
 * @param {string} agent - 'claude-code', 'pi-coding-agent', 'gemini-cli', 'codex-cli', 'opencode', 'cursor-agent'
...
function buildAgentAuthEnv(agent) {
  ...
  if (agent === 'claude-code') {
    ...
    backendApi = 'google';
    const authMode = getConfig('CODING_AGENT_GEMINI_CLI_AUTH') || 'api-key';
    if (authMode === 'oauth') {
      const oauthCreds = getConfig('GEMINI_OAUTH_CREDS');
      if (oauthCreds) env.push(`GEMINI_OAUTH_CREDS=${oauthCreds}`);
    } else {
    ...
    backendApi = 'cursor';
    const authMode = getConfig('CODING_AGENT_CURSOR_AUTH') || 'api-key';
    if (authMode === 'api-key') {
      const key = getConfig('CURSOR_API_KEY');
      if (key) env.push(`CURSOR_API_KEY=${key}`);
    }
    const model = getConfig('CODING_AGENT_CURSOR_MODEL');
    if (model) env.push(`LLM_MODEL=${model}`);
  }
...
/**
 * Build credential bind mounts for agents that use host-side OAuth.
 * Mounts read-only credential files from VPS host into container.
 */
function buildCredentialBinds() {
  const binds = [];
  // Gemini CLI OAuth credentials
  const geminiCreds = '/root/.gemini/oauth_creds.json';
  if (fs.existsSync(geminiCreds)) {
    binds.push(`${geminiCreds}:/home/coding-agent/.gemini/oauth_creds.json:ro`);
    // Also mount google_accounts.json if present
    const geminiAccounts = '/root/.gemini/google_accounts.json';
    if (fs.existsSync(geminiAccounts)) {
      binds.push(`${geminiAccounts}:/home/coding-agent/.gemini/google_accounts.json:ro`);
    }
  }
  // Cursor Agent credentials
  const cursorDir = '/root/.local/share/cursor-agent';
  if (fs.existsSync(cursorDir)) {
    binds.push(`${cursorDir}:/home/coding-agent/.local/share/cursor-agent:ro`);
  }
  // Cursor Agent CLI binary (if installed on host)
  const cursorBin = '/root/.local/bin/cursor-agent';
  if (fs.existsSync(cursorBin)) {
    binds.push(`${cursorBin}:/usr/local/bin/cursor-agent:ro`);
  }
  return binds;
}
...
async function runCommandContainer({ containerName, command, workspaceId, repo, branch, featureBranch, prompt, draft, codingAgent }) {
  ...
  const hostDir = await resolveHostPath(dir);
  const hostConfig = {
    Binds: [`${hostDir}:/home/coding-agent`, ...buildCredentialBinds()],
  };
...
async function runAgentJobContainer({ agentJobId, repo, branch, title, description, codingAgent, llmModel }) {
  ...
  const result = await runContainer({
    ...
    hostConfig: {
      AutoRemove: true,
      Binds: [`${volumeName}:/home/coding-agent`, ...buildCredentialBinds()],
      Memory: 2 * 1024 * 1024 * 1024,
      MemorySwap: 2 * 1024 * 1024 * 1024,
      CpuShares: 1024,
      PidsLimit: 512,

...
export async function getWorkspaceDiffStats(id, authenticatedUser) {
  ...
  try {
    ...
    let currentBranch;
    try {
      const { stdout } = await execAsync(`git -c safe.directory='*' rev-parse --abbrev-ref HEAD`, opts);
    ...
    if (currentBranch) {
      try {
        await execAsync(`git -c safe.directory='*' rev-parse --verify origin/${currentBranch} 2>/dev/null`, opts);
    ...
    for (const file of untracked.split('\n')) {
      if (!file) continue;
      try {
        const { stdout: countOut } = await execAsync(`grep -c '' "${file}"`, opts);
...
export async function getWorkspaceDiffFull(id, authenticatedUser) {
  ...
  try {
    ...
    for (const file of untrackedFiles.split('\n')) {
      if (!file) continue;
      try {
        const content = await fs.promises.readFile(path.join(repoPath, file), 'utf8');

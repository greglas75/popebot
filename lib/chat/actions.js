...
import {
  ...
  updateChatTitle,
  toggleChatStarred,
  archiveChat as dbArchiveChat,
  unarchiveChat as dbUnarchiveChat,
  getArchivedChats as dbGetArchivedChats,
} from '../db/chats.js';
import {
...
export async function getChats(limit) {
  ...
  let query = db
    ...
    .where(and(
      or(eq(chats.userId, user.id), eq(chats.userId, 'telegram')),
      eq(chats.archived, 0),
    ))
...
/**
 * Archive a chat (with ownership check).
 * @param {string} chatId
 * @returns {Promise<{success: boolean}>}
 */
export async function archiveChat(chatId) {
  ...
  dbArchiveChat(chatId);
  return { success: true };
}

/**
 * Unarchive a chat (with ownership check).
 * @param {string} chatId
 * @returns {Promise<{success: boolean}>}
 */
...
export async function getArchivedChats() {
  const user = await requireAuth();
  return dbGetArchivedChats(user.id);
}
...
export async function generateChatTitle(chatId, firstMessage) {
  const user = await requireAuth();
  const chat = getChatById(chatId);
  if (chat && chat.userId !== user.id && chat.userId !== 'telegram') return null;
...
export async function stopDockerContainer(containerName) {
  const user = await requireAuth();
  try {
    const { getCodeWorkspaceByContainerName } = await import('../db/code-workspaces.js');
    const ws = getCodeWorkspaceByContainerName(containerName);
    if (!ws || (ws.userId !== user.id && user.role !== 'admin')) return { error: 'Not found' };
...
/**
 * Get list of available (enabled + ready) coding agents for the chat UI selector.
 */
export async function getAvailableAgents() {
  ...
  try {
    ...
    const defaultAgent = getConfig('CODING_AGENT') || 'claude-code';
    const agents = [];

    if (getConfig('CODING_AGENT_CLAUDE_CODE_ENABLED') === 'true') {
      const auth = getConfig('CODING_AGENT_CLAUDE_CODE_AUTH') || 'oauth';
      const backend = getConfig('CODING_AGENT_CLAUDE_CODE_BACKEND') || 'anthropic';
      const isOAuth = backend === 'anthropic' && auth === 'oauth';
      const apiTag = isOAuth ? '' : ' (API)';
      let ready = false;
      if (backend === 'anthropic') {
        ready = isOAuth
          ? getOAuthTokenCount('claudeCode') > 0
          : (getSecretStatus(['ANTHROPIC_API_KEY'])[0]?.isSet || false);
      } else {
        ready = true;
      }
      if (ready) agents.push({
        value: 'claude-code', label: 'Claude Code',
        models: [
          { value: 'claude-sonnet-4-6', label: `Sonnet 4.6${apiTag}` },
          { value: 'claude-opus-4-6', label: `Opus 4.6${apiTag}` },
          { value: 'claude-haiku-4-5-20251001', label: `Haiku 4.5${apiTag}` },
        ],
      });
    }

    if (getConfig('CODING_AGENT_GEMINI_CLI_ENABLED') === 'true') {
      const googleKeySet = getSecretStatus(['GOOGLE_API_KEY'])[0]?.isSet || false;
      if (googleKeySet) agents.push({
        value: 'gemini-cli', label: 'Gemini CLI',
        models: [
          { value: 'gemini-2.5-pro', label: 'Gemini 2.5 Pro (API)' },
          { value: 'gemini-2.5-flash', label: 'Gemini 2.5 Flash (API)' },
        ],
      });
    }

    if (getConfig('CODING_AGENT_CODEX_CLI_ENABLED') === 'true') {
      const auth = getConfig('CODING_AGENT_CODEX_CLI_AUTH') || 'api-key';
      const codexOAuth = auth === 'oauth';
      const codexApiTag = codexOAuth ? '' : ' (API)';
      const ready = codexOAuth
        ? getOAuthTokenCount('codex') > 0
        : (getSecretStatus(['OPENAI_API_KEY'])[0]?.isSet || false);
      if (ready) agents.push({
        value: 'codex-cli', label: 'Codex CLI',
        models: [
          { value: 'o3', label: `o3${codexApiTag}` },
          { value: 'o4-mini', label: `o4-mini${codexApiTag}` },
          { value: 'gpt-4.1', label: `GPT-4.1${codexApiTag}` },
        ],
      });
    }

    if (getConfig('CODING_AGENT_OPENCODE_ENABLED') === 'true' && getConfig('CODING_AGENT_OPENCODE_PROVIDER')) {
      agents.push({ value: 'opencode', label: 'OpenCode (API)', models: [] });
    }

    if (getConfig('CODING_AGENT_KIMI_CLI_ENABLED') === 'true' && getConfig('CODING_AGENT_KIMI_CLI_PROVIDER')) {
      agents.push({ value: 'kimi-cli', label: 'Kimi CLI (API)', models: [] });
    }

    return { defaultAgent, agents };
  } catch (err) {
    console.error('Failed to get available agents:', err);
    return { defaultAgent: 'claude-code', agents: [{ value: 'claude-code', label: 'Claude Code', models: [] }] };
  }
}

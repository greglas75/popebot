...
import { rateLimit, rateLimitResponse } from '../rate-limit.js';
...
export async function POST(request) {
  ...
  const rl = rateLimitResponse(rateLimit(`chat:${session.user.id}`, 20, 60_000));
  if (rl) return rl;
  ...
  const { messages, chatId: rawChatId, trigger, codeMode, repo, branch, workspaceId, codeModeType, codingAgent: requestedAgent, codingModel: requestedModel, persistentContainer } = body;
  ...
  for (const part of fileParts) {
    ...
    if (!ALLOWED_MEDIA_TYPES.has(mediaType)) continue;
  ...
  const stream = createUIMessageStream({
    ...
    execute: async ({ writer }) => {
      ...
      const streamOptions = {
        ...
        codingAgent: requestedAgent || undefined,
        codingModel: requestedModel || undefined,
      };
      ...
      for await (const chunk of chunks) {
        if (chunk.type === 'text') {
          ...
          if (chunk.toolName === 'coding_agent') {
            const agent = requestedAgent || getConfig('CODING_AGENT') || 'claude-code';
            const providerKeys = {
...
export async function getChatsHandler(request) {
  ...
  let query = db
    ...
    .where(and(
      or(eq(chats.userId, session.user.id), eq(chats.userId, 'telegram')),
      eq(chats.archived, 0),
    ))
...
/**
 * GET handler for /projects/list — user's projects with recent chats and container status.
 */
export async function getProjectsListHandler(request) {
  const session = await auth();
  if (!session?.user?.id) {
    return Response.json({ error: 'Unauthorized' }, { status: 401 });
  }

  const { getProjectsByUser } = await import('../db/projects.js');
  const { eq, desc, and } = await import('drizzle-orm');
  const { getDb } = await import('../db/index.js');
  const { chats, codeWorkspaces } = await import('../db/schema.js');

  const projects = getProjectsByUser(session.user.id);
  const db = getDb();

  const enriched = projects.map((project) => {
    // Query chats for this project, joined with workspace for container status
    const projectChats = db
      .select({
        ...
        updatedAt: chats.updatedAt,
        containerName: codeWorkspaces.containerName,
        containerStatus: codeWorkspaces.containerStatus,
      })
      .from(chats)
      .leftJoin(codeWorkspaces, eq(chats.codeWorkspaceId, codeWorkspaces.id))
      .where(and(eq(chats.projectId, project.id), eq(chats.userId, session.user.id), eq(chats.archived, 0)))
      .orderBy(desc(chats.updatedAt))
      .limit(4)
      .all();
    ...
    return {
      ...
      recentChats: projectChats.slice(0, 3).map((c) => ({
        id: c.id,
        title: c.title,
        updatedAt: c.updatedAt,
        containerName: c.containerName || null,
        containerStatus: c.containerStatus || null,
      })),
    };
  });

  // Also include loose chats (no projectId) as a virtual "Recent" group
  ...
  return Response.json({ projects: enriched, looseChats: looseChats.map((c) => ({
    id: c.id,
    title: c.title,
    updatedAt: c.updatedAt,
    containerName: c.containerName || null,
...
/**
 * POST handler for /chat/git-action — execute git commands on the persistent container.
 * Actions: commit, push, create-pr, create-branch.
 */
export async function postGitActionHandler(request) {
  const session = await auth();
  if (!session?.user?.id) {
    return Response.json({ error: 'Unauthorized' }, { status: 401 });
  }
  ...
  const { workspaceId, action, message } = await request.json();
  if (!workspaceId || !action) {
    return Response.json({ error: 'workspaceId and action required' }, { status: 400 });
  }
  ...
  if (!workspace.containerName) {
    return Response.json({ error: 'No active container for this workspace' }, { status: 400 });
  }

  const { execInContainer } = await import('../tools/docker.js');
  const wsDir = '/home/coding-agent/workspace';

  let cmd;
  switch (action) {
    case 'commit': {
      if (!message) return Response.json({ error: 'Commit message required' }, { status: 400 });
      const safeMsg = message.replace(/'/g, "'\\''");
      cmd = `cd ${wsDir} && git add -A && git commit -m '${safeMsg}'`;
      break;
    }
    case 'push':
      cmd = `cd ${wsDir} && git push`;
      break;
    case 'create-pr':
      cmd = `cd ${wsDir} && gh pr create --fill`;
      break;
    case 'create-branch': {
      if (!message) return Response.json({ error: 'Branch name required' }, { status: 400 });
      const safeBranch = message.replace(/[^a-zA-Z0-9._/-]/g, '-');
      cmd = `cd ${wsDir} && git checkout -b '${safeBranch}'`;
      break;
    }
    default:
  ...
  try {
    const output = await execInContainer(workspace.containerName, cmd, 60000);
    if (output === null) {
      return Response.json({ success: false, output: 'Command timed out or failed' }, { status: 504 });
    }
    return Response.json({ success: true, output: output || '' });
  } catch (err) {
    console.error(`[git-action] ${action} failed:`, err.message);
    return Response.json({ success: false, output: 'Git operation failed' }, { status: 500 });
  }
}
...
export async function finalizeChat(request) {
  ...
  const { getChatById: getChatForOwnerCheck } = await import('../db/chats.js');
  const chatRecord = getChatForOwnerCheck(chatId);
  if (!chatRecord || (chatRecord.userId !== session.user.id && chatRecord.userId !== 'telegram')) {
  ...
  // Look up linked workspace (if code chat) — reuse chatRecord from ownership check
  let codeWorkspaceId = null;
  let featureBranch = null;
  try {
    if (chatRecord?.codeWorkspaceId) {
      codeWorkspaceId = chatRecord.codeWorkspaceId;
      const { getCodeWorkspaceById } = await import('../db/code-workspaces.js');

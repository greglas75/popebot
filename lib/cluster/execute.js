...
export async function runClusterRole(roleData, payload, trigger) {
  ...
  let payloadPrompt = payload?.prompt;

  // Validate and sanitize payload prompt
  if (payloadPrompt !== undefined && payloadPrompt !== null) {
    if (typeof payloadPrompt !== 'string') {
      throw new Error(`[cluster] Invalid payload: prompt must be a string, got ${typeof payloadPrompt}`);
    }
    if (payloadPrompt.length > 50000) {
      throw new Error(`[cluster] Invalid payload: prompt exceeds maximum length of 50000 characters (got ${payloadPrompt.length})`);
    }
    // Strip null bytes
    payloadPrompt = payloadPrompt.replace(/\0/g, '');
  }

  // Validate template variable values in payload to prevent injection
  if (payload && typeof payload === 'object') {
    for (const [key, val] of Object.entries(payload)) {
      if (key === 'prompt') continue;
      if (typeof val === 'string' && (val.includes('{{') || val.includes('}}'))) {
        console.warn(`[cluster] Payload variable '${key}' contains template injection characters — sanitizing`);
        payload[key] = val.replace(/\{\{/g, '{ {').replace(/\}\}/g, '} }');
      }
    }
  }

  const cleanPayload = payload && payloadPrompt !== undefined

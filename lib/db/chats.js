 ...
 * @param {object} [options] - { chatMode: 'agent' | 'code' }
...
/**
 * Get all non-archived chats for a user, ordered by most recently updated.
 * @param {string} userId
 * @returns {object[]}
 */
export function getChatsByUser(userId) {
  ...
  return db
    .select()
    .from(chats)
    .where(and(eq(chats.userId, userId), eq(chats.archived, 0)))
    .orderBy(desc(chats.updatedAt))
    .all();
}

/**
 * Get all archived chats for a user, ordered by most recently updated.
 * @param {string} userId
 * @returns {object[]}
 */
export function getArchivedChats(userId) {
  const db = getDb();
  return db
    .select()
    .from(chats)
    .where(and(eq(chats.userId, userId), eq(chats.archived, 1)))
    .orderBy(desc(chats.updatedAt))
    .all();
}
...
export function linkChatToProject(chatId, projectId) {
  const db = getDb();
  db.update(chats)

...
export function getCustomProviders() {
  ...
  return rows.map((row) => {
    try {
      ...
      console.warn(`[config] failed to decrypt provider "${row.key}":`, err instanceof Error ? err.message : String(err));
      return null;
    }
  }).filter(Boolean);
}
...
export function getCustomProvider(key) {
  ...
  try {
    ...
    console.warn(`[config] failed to decrypt provider "${key}":`, err instanceof Error ? err.message : String(err));
    return null;
  }
}

...
/**
 * Look up a code workspace by its Docker container name.
 * @param {string} containerName
 * @returns {object|undefined}
 */
export function getCodeWorkspaceByContainerName(containerName) {
  const db = getDb();
  return db.select().from(codeWorkspaces).where(eq(codeWorkspaces.containerName, containerName)).get();
}
...
const VALID_CONTAINER_STATUSES = ['none', 'starting', 'ready', 'paused', 'stopped'];

/**
 * Update the container status on a workspace.
 * Also sets containerStartedAt when transitioning to 'starting'.
 * @param {string} id - Workspace ID
 * @param {'none'|'starting'|'ready'|'paused'|'stopped'} status - Container lifecycle status
 */
export function updateContainerStatus(id, status) {
  if (!VALID_CONTAINER_STATUSES.includes(status)) {
    throw new Error(`Invalid container status: ${status}`);
  }
  const db = getDb();
  const fields = { containerStatus: status, updatedAt: Date.now() };
  if (status === 'starting') {
    fields.containerStartedAt = Date.now();
  }
  db.update(codeWorkspaces)
    .set(fields)
    .where(eq(codeWorkspaces.id, id))
...
/**
 * Update the last message timestamp on a workspace (marks recent activity).
 * @param {string} id - Workspace ID
 */
export function updateLastMessageAt(id) {
  const db = getDb();
  db.update(codeWorkspaces)
    .set({ lastMessageAt: Date.now(), updatedAt: Date.now() })
    .where(eq(codeWorkspaces.id, id))
    .run();
}

/**
 * Get all workspaces that have an active container (status is not 'none').
 * Used by the idle management system to determine which containers to pause/stop.
 * @returns {object[]}
 */
export function getWorkspacesForIdleManagement() {
  const db = getDb();
  return db
    .select()
    .from(codeWorkspaces)
    .where(ne(codeWorkspaces.containerStatus, 'none'))
    .limit(1000)
    .all();
}
